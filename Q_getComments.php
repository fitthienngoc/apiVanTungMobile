...
if($_SERVER['PHP_AUTH_USER']){
	...
	if ($data['permission'] == 'admin') {
		...
		$search = "%"."$search"."%";
		$sql2="SELECT * FROM `comment` WHERE `content` LIKE '$search' OR `fullName` LIKE '$search'";
		...
		$sql3="SELECT * FROM `comment` WHERE `content` LIKE '$search' OR `fullName` LIKE '$search'  ORDER BY `comment`.`Id` DESC LIMIT $start, $limit  ";
