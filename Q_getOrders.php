...
if ($_SERVER['PHP_AUTH_USER']) {
	...
	if ($data['permission'] == 'admin') {
		...
		$search = "%" . "$search" . "%";
		$sql2 = "SELECT * FROM `orders` WHERE `dataUser` LIKE '$search' OR `dataBill` LIKE '$search'";
		...
		$sql3 = "SELECT * FROM `orders` WHERE `dataUser` LIKE '$search' OR `dataBill` LIKE '$search'  ORDER BY `orders`.`Id` DESC LIMIT $start, $limit  ";
